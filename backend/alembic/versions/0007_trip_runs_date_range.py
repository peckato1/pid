"""trip_runs: replace p_date with p_from_date/p_to_date range
...
_BODY = """
    ...
    WHERE (p_from_date   IS NULL OR rtr.start_date        >= p_from_date)
      AND (p_to_date     IS NULL OR rtr.start_date        <= p_to_date)
...
def upgrade() -> None:
    ...
    op.execute(f"""
        ...
        LANGUAGE sql STABLE AS $${_BODY}$$;
    """)


def downgrade() -> None:
    op.execute("DROP FUNCTION IF EXISTS trip_runs(DATE, DATE, TEXT, TEXT, INT, INT)")
    op.execute(f"""
        CREATE FUNCTION trip_runs(
